...
def gen_Lagrange_coeffs(alpha_s, beta_s, p, is_K1=0):
    ...
    #         U = [[0_truncat for col in range(len(beta_s))] for row in range(len(alpha_s))]
    # print(alpha_s)
    # print(beta_s)
...
def BGW_decoding(f_eval, worker_idx, p):  # decode the output from T+1_truncat evaluation points
    # f_eval     : [RT X d ]
    # worker_idx : [ 1_truncat X RT]
    # output     : [ 1_truncat X d ]
...
def LCC_decoding(f_eval, f_deg, N, K, T, worker_idx, p):
    # RT_LCC = f_deg * (K + T - 1_truncat) + 1_truncat
...
def Gen_Additive_SS(d, n_out, p):
    ...
    # print np.mod(np.sum(Additive_SS,axis=0_truncat),p)
