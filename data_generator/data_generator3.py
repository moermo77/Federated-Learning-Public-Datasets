...
def data_generator(client_num, clinet_data_size, func, noise, mean, sigma, share = False, \
    ...
    for i in range(client_num):
        ...
        if share:
            ...
            index = np.random.choice(data.shape[0],shared_data_size,replace=False)
            shared.append(np.concatenate([np.expand_dims(data[index], 1), \
                                          np.expand_dims(label[index], 1),
                                          np.expand_dims(np.ones_like(label[index]) * i, 1)], 1))
    ...
    if share:
        ...
        for i in range(client_num):
            ...
            train_data[i] = np.append(train_data[i], shared[index,0])
            train_label[i] = np.append(train_label[i], shared[index,1])
    ...
    if share:
        shared = np.concatenate(shared,0)
        for i in range(client_num):
            avaliable_shared = shared[shared[:,2]!=i]
            index = np.random.choice(avaliable_shared.shape[0],shared_data_size,replace=False)
            train_data[i] = np.append(train_data[i], avaliable_shared[index,0])
            train_label[i] = np.append(train_label[i], avaliable_shared[index,1])
            print(i,'共享数据来自',np.unique(avaliable_shared[index,2]),'数目',[ np.sum(avaliable_shared[index,2]==i) for i in np.unique(avaliable_shared[index,2])])
            avaliable_shared = np.delete(avaliable_shared,index,0)
            shared = np.concatenate([avaliable_shared, shared[shared[:,2]==i]])
    #可视化数据
    plot_data(train_data, train_label)

    #导出数据
    np.savetxt(os.path.join(train_dir,"train_data.txt"),np.concatenate(train_data).reshape(-1,1))
    np.savetxt(os.path.join(train_dir,"train_label.txt"),np.concatenate(train_label).reshape(-1,1))
    np.savetxt(os.path.join(test_dir,"test_data.txt"),np.concatenate(test_data).reshape(-1,1))
    np.savetxt(os.path.join(test_dir,"test_label.txt"),np.concatenate(test_label).reshape(-1,1))
    ...
    for i in range(client_num):
        train_idx[i] = np.linspace(i*len(train_data[i]),(i+1)*len(train_data[i])-1,len(train_data[i])).astype(np.int).tolist()
        test_idx[i] = np.linspace(i*len(test_data[i]),(i+1)*len(test_data[i])-1,len(test_data[i])).astype(np.int).tolist()
...
if __name__ == "__main__":
    ...
    if_share = True
    share_rate = 0.03
    data_mean = [-0.5,0.5,1.5,2.5,3.5]
    data_sigma = np.ones(client_num)*0.1
    ...
    data_generator(client_num, client_data_size, func, noise, data_mean, \
                   data_sigma, if_share, shared_data_size = int(share_rate*client_data_size), data_lower_bound=-2, \
